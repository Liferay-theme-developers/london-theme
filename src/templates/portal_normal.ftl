...
<div class="container-fluid" id="wrapper">
		...
		<header id="banner" role="banner">
			<div id="heading">
				<h1 class="site-title">
					...
					<#if show_site_name>						<span class="site-name" title="<@liferay.language_format arguments="${site_name}" key="go-to-x" />
							${site_name}
						</span>
					</#if>				</h1>
			</div>

			<#if !is_signed_in>				<a data-redirect="${is_login_redirect_required}" href="${sign_in_url}" id="sign-in" rel="nofollow">${sign_in_text}</a>
			</#if>
			<#if has_navigation && is_setup_complete>				<#include full_templates_path + "/navigation.ftl">			</#if>		</header>
	...
	<#include full_templates_path + "/footer.vm">
</div>
